...
#    base_MCF=1;
    ...
    for i=1:length(TXA_vals)
        y = initialConditions(94,TXA_vals[i]);
        problem(t,y,ydot) = massBalances(t,y,ydot,vars);
        X = Sundials.cvode(problem,y,TSIM,reltol=1e-7,abstol=1e-18);
#        MCF,MCF_ind=findmax(X[:,69]);
    ...
    writedlm("./results_dump/test_apr11_1.csv",sol,',');
    return
end
...
#    vars["constants"]=[vars["constants"] 0.313]
    ...
    TSIM = convert(Array{Float64,1}, TSIM)
    X=0;
    y = initialConditions(94,TXA_val);
    problem(t,y,ydot) = massBalances(t,y,ydot,vars);
    X = Sundials.cvode(problem,y,TSIM,reltol=1e-6,abstol=1e-18);

    writedlm("./results_dump/test_apr11_full_1.csv",X,',');
    return(vars)
end
