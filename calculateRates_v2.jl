function calculateRates(t,y,data_dictionary)
	...
	RxnRates=Array{Float64,1}(undef,185);
	...
	RxnRates[121,1]=Kvalues[121]*y[93]; 		# r[121] = Pn_FnII degradation of FnII
	RxnRates[122,1]=Kvalues[122]*y[94]; 		# r[122] = k81*Pn_FnII2 degradation of FnII2
	RxnRates[123,1]=Kvalues[123]*y[79]*y[39]; 		# r[123] = k81*PAI*APC
	RxnRates[124,1]=Kvalues[124]; 		# r[124] = k83 artpa production (deactivated)
	...
	RxnRates[129,1]=Kvalues[129]*y[95]*y[84]/(2.5e-5+y[95]);	# upa activation of Pgx, decreases Pgx, increases Pnx
	RxnRates[130,1]=Kvalues[130]*y[81]*y[86];			# binding of TXA to Pg, decreases TXA and Pg, increases Pgx
	RxnRates[131,1]=Kvalues[131]*y[95]; 				# unbinding of Pgx, decreases Pgx, increases TXA and Pg
	RxnRates[132,1]=Kvalues[132]*y[84]*y[79];				#PAI-1 and uPA binding, decreases PAI-1 and uPA
	RxnRates[133,1]=Kvalues[133]*y[74]*y[86];			# binding of TXA to Pn, decreases TXA and Pn, increases Pnx
	RxnRates[134,1]=Kvalues[134]*y[99]; 				# unbinding of Pn_TXA, decreases Pnx, increases TXA and Pn
	...
	RxnRates[140,1]=Kvalues[140]*y[89]*y[91]/(y[91]+0.19e-6); 		# r[148] = k73*stPA_FnII and Pg_FnII forms Pn_FnII
	RxnRates[141,1]=Kvalues[141]*y[90]*y[92]/(y[92]+0.19e-6); 		# r[149] = k67*stPA_FnII2 and Pg_FnII2 forms Pn_FnII2
	RxnRates[142,1]=Kvalues[142]*y[65]*y[74];		# r[150] = k79*FnII*Pn
	RxnRates[143,1]=Kvalues[143]*y[69]*y[74]; 		# r[151] = k78*FnII2*Pn
	RxnRates[144,1]=Kvalues[144]*y[93]; 		# r[152] = k80*Pn_FnII
	RxnRates[145,1]=Kvalues[145]*y[94]; 		# r[153] = k90*Pn_FnII2 unbinding
	RxnRates[146,1]=Kvalues[146]*y[74]*y[60]; #fibrinogenolysis
	RxnRates[147,1]=Kvalues[147]*y[95]*y[86]; #binding Pgx with TXA forming Pgxy
	RxnRates[148,1]=Kvalues[148]*y[97]; #unbinding Pgxy forming TXA and Pgx
	RxnRates[149,1]=Kvalues[149]*y[81]*y[86];			# binding of TXA to Pg, decreases TXA and Pg, increases Pgy
	RxnRates[150,1]=Kvalues[150]*y[96]; 				# unbinding of Pgy, decreases Pgy, increases TXA and Pg
	RxnRates[151,1]=Kvalues[151]*y[96]*y[86]; #binding Pgy with TXA forming Pgxy
	RxnRates[152,1]=Kvalues[152]*y[97]; #unbinding Pgxy forming TXA and Pgx
	RxnRates[153,1]=Kvalues[153]*y[95]*y[69]; 		# r[137] = k70*Pgx*FnII2
	RxnRates[154,1]=Kvalues[154]*y[98]; 		# r[139] = k71*Pgx_FnII2
	RxnRates[155,1]=Kvalues[155]*y[69]*y[99]; 		# r[151] = k78*FnII2*Pnx
	RxnRates[156,1]=Kvalues[156]*y[101]; 		# r[153] = k90*Pnx_FnII2
	RxnRates[157,1]=Kvalues[157]*y[97]*y[84]/(2.5e-5+y[97]);	# upa activation of Pgxy, decreases Pgxy, increases Pnx and TXA
	RxnRates[158,1]=Kvalues[158]*y[74]*y[86];			# binding of TXA to Pn, decreases TXA and Pn, increases Pnz
	RxnRates[159,1]=Kvalues[159]*y[102]; 				# unbinding of Pnz, decreases Pnz, increases TXA and Pn
	RxnRates[160,1]=Kvalues[160]*y[99]*y[86]; #binding Pnx with TXA forming Pnxz
	RxnRates[161,1]=Kvalues[161]*y[100]; #unbinding Pnxz forming TXA and Pnx
	RxnRates[162,1]=Kvalues[162]*y[102]*y[86]; #binding Pnz with TXA forming Pnxz
	RxnRates[163,1]=Kvalues[163]*y[100]; #unbinding Pnxz forming TXA and Pnz
	RxnRates[164,1]=Kvalues[164]*y[96]*y[84]/(2.5e-5+y[96]);	# upa activation of Pgy, decreases Pgy, increases Pn and TXA
	RxnRates[165,1]=Kvalues[165]*y[90]*y[98]/(y[98]+0.19e-6); 		# tpa activation of Pgx_FnII2
	RxnRates[166,1]=Kvalues[166]*y[101]; 		# Pnx_FnII2 degrading fibrin increases FDP, Pnx
	RxnRates[167,1]=Kvalues[116]*y[99]*y[75]/40; 		# r[116] = k77*Pnx*AP
	RxnRates[168,1]=Kvalues[116]*y[102]*y[75]/40; 		# r[116] = k77*Pnz*AP
	RxnRates[169,1]=Kvalues[116]*y[100]*y[75]/40; 		# r[116] = k77*Pnxz*AP
	RxnRates[170,1]=Kvalues[116]*y[94]*y[75]/40;		# r[116] = k77*Pn_FnII2*AP
	RxnRates[171,1]=Kvalues[116]*y[101]*y[75]/40;		# r[116] = k77*Pnx_FnII2*AP
	RxnRates[172,1]=Kvalues[128]*y[92]*y[84]/(2.5e-5+y[81]);	 # upa activation of Pg_FnII2, decreases Pg_FnII2, increases PN_FnII2
	RxnRates[173,1]=Kvalues[129]*y[98]*y[84]/(2.5e-5+y[95]);	# upa activation of Pgx_FnII2, Pgx_FnII2, increases Pnx_FnII2
	RxnRates[174,1]=Kvalues[167]*y[103]*y[84]; #PCI inhibition of uPA, decreases PCI, uPA
	RxnRates[175,1]=Kvalues[116]*y[74]*y[104]/40; 		# r[116] = k77*Pn*APnc
	RxnRates[176,1]=Kvalues[116]*y[99]*y[104]/160; 		# r[116] = k77*Pnx*APnc
	RxnRates[177,1]=Kvalues[116]*y[101]*y[104]/160;		# r[116] = k77*Pnx_FnII2*APnc

	RxnRates[178,1]=Kvalues[168]*y[74]*y[105]; 		# r[116] = k77*Pn*a2mg
	RxnRates[179,1]=Kvalues[168]*y[99]*y[105]; 		# r[116] = k77*Pnx*a2mg
	RxnRates[180,1]=Kvalues[168]*y[102]*y[105]; 		# r[116] = k77*Pnz*a2mg
	RxnRates[181,1]=Kvalues[168]*y[100]*y[105]; 		# r[116] = k77*Pnxz*a2mg

	RxnRates[182,1]=Kvalues[169]*y[74]*y[106]; 		# r[116] = k77*Pn*a1at
	RxnRates[183,1]=Kvalues[169]*y[99]*y[106]; 		# r[116] = k77*Pnx*a1at
	RxnRates[184,1]=Kvalues[169]*y[102]*y[106]; 		# r[116] = k77*Pnz*a1at
	RxnRates[185,1]=Kvalues[169]*y[100]*y[106]; 		# r[116] = k77*Pnxz*a1at
	return RxnRates;
end
